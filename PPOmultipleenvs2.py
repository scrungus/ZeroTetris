...
class PPOLightning(LightningModule):
    ...
    def __init__(
        ...
        for _ in range(self.env_count):
            self.envs.append(Tetris(grid_dims=(10, 10), piece_size=2))
            self.states.append(torch.Tensor(self.env.reset()))
        ...
        self.batch_states = tuple([] for _ in range(self.env_count))
        self.batch_actions = tuple([] for _ in range(self.env_count))
        self.batch_probs = tuple([] for _ in range(self.env_count))
        self.batch_advs = tuple([] for _ in range(self.env_count))
        self.batch_vals = tuple([] for _ in range(self.env_count))
        self.ep_rewards = tuple([] for _ in range(self.env_count))
        self.ep_vals = tuple([] for _ in range(self.env_count))
        self.epoch_rewards = []
        self.avg_reward = 0
