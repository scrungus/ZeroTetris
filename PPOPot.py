...
class PPOLightning(LightningModule):
    ...
    def make_batch(self):
        for i in range(self.hparams.epoch_steps):
            ...
            next_state, reward, lines_cleared, done, _ = self.env.step(action.item())
            self.ep_step += 1
            ...
            #logging
            self.ep_lines_cleared.append(lines_cleared)
            ...
            if done or end:
                ...
                #append bootstrapped value to lines cleared to maintain comparability to other experiments
                self.ep_lines_cleared.append(next_val)

                self.epoch_rewards.append(sum(self.ep_lines_cleared))
                #print("Total for Ep :",sum(self.ep_rewards))
                self.ep_lines_cleared.clear()
...
with torch.no_grad():
    for i in range(10):
        ...
        while not done:
            ...
            state, reward,lines_cleared, done, _ = env.step(action.item())
            total += lines_cleared
