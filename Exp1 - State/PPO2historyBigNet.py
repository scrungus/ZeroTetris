...
class CriticNet(nn.Module):
    def __init__(self, obs_size, hidden_size = 100*4):
        super().__init__()
        print(obs_size)
...
class ActorNet(nn.Module):
    def __init__(self, obs_size, n_actions, depth, hidden_size = 64*4):
        super().__init__()
...
class PPOLightning(LightningModule):
    ...
    def __init__(
        ...
        self.state.append(torch.Tensor(np.ones(obs_size)))
        ...
        self.critic = CriticNet(obs_size*2)
        self.actor = ActorNet(obs_size*2,n_actions,self.hparams.depth)
    ...
    def make_batch(self):
        for i in range(self.hparams.epoch_steps):
            ...
            self.state.append(torch.Tensor(next_state))
